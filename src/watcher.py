...
class Watcher:
    ...
    async def watch(self, watchidlist: list[int], delaytime_s: int, iterations: int=0):
        iter_count = 0
        while iterations <= 0 or iter_count < iterations:
            ...
            iter_count += 1
            self._logger.debug('Sleeping for %s seconds', delaytime_s)
    ...
    async def _perform_user_log(self, session: aiohttp.ClientSession, userId: str) -> None:
        ...
        try:
            pageresult = await self.get_user_page(session, userId)
    ...
    async def get_user_page(self, session: aiohttp.ClientSession, id: str) -> Result[OfflineStatsPage, str]:
        url = self._urlgenerator.get_stats(id)
        
        async with session.get(url) as resp:
            self._logger.debug('Received response %s for id %s', resp.status, id)
            ...
            if pageresult.is_err:
                self._logger.warning('Failed to lookup user ID %s: %s', pageresult.unwrap_err())
                return pageresult
